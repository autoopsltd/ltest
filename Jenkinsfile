        ...
        stage('mocha/istanbul') {
            ...
            post {
                success {
                    ...
                    junit '**/artifacts/**/*.xml'
                    publishHTML([reportDir: coverage, reportFiles: 'index.html', reportName: 'Istanbul'])
                }
                failure {
        ...
        stage('sonarqube') {
            ...
            steps {
                withSonarQubeEnv('sonarqube') {
                    sh '${sonarScannerHome}/bin/sonar-scanner -e -Dsonar.projectName=ltest -DsonarprojectVersion=${env.BUILD_NUMBER} -Dsonar.sources=. -Dsonar.language=js'
                }
            }
            post {
                success {
